...
- Web lab `Prompt one` is a blind multi-turn A/B eval: ten connected prompts run one-by-one through regular messages and StateWeave, the user votes A/B/both/neither against expected behavior, and labels are revealed only after all votes.
