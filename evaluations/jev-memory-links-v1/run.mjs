...
assert.equal(cases.length,36);assert.equal(new Set(cases.map(p=>p.id)).size,36);
...
const hashes={};for(const file of ['PROTOCOL.md','cases.json','linker.mjs','graph.mjs','run.mjs'])hashes[file]=sha(await readFile(new URL(file,import.meta.url)));
await save('protocol',{protocol,commit:process.env.PROBE_COMMIT,hashes,startedAt:new Date().toISOString()});
const ordered=[...cases].sort((a,b)=>sha('links-v1'+a.id).localeCompare(sha('links-v1'+b.id)));
...
const variants=[[cases.slice(0,6),8],[cases.slice(6,12),8],[cases.slice(0,12),16]];
