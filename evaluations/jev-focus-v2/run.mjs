...
const bytes=await readFile(new URL('../jev-focus-v1/cases.json',import.meta.url));
...
const protocol={id:'jev-focus-diagnostic-v2',fixtureSha256:hash,cases:cases.length,arms,answerModel:process.env.ANTHROPIC_MODEL,judgeModel:'jev-1.13.0',maxIterations:5,maxNodes:16,targetTokens:16000,maxTokens:64000,decoding:{temperature:0,maxOutputTokens:4096},order:'Latin rotation by case index',startedAt:new Date().toISOString()};
...
for(const [index,test] of cases.entries()){
 ...
 for(let position=0;position<3;position++){
  ...
  const agent=new Agent({model,tools,state:structuredClone(state),contextMode:'molecular',projectionMaxNodes:16,projectionTargetTokens:16000,maxPromptTokens:64000,maxIterations:5,
   systemPrompt:'Use the supplied project records. Respect the latest corrections and revocations. Do not guess unavailable values. Return the requested decision and the verification marker in ordinary final-answer text.',
   ...(arm==='deterministic'?{}:{focusReranker:createJevFocusReranker({apiKey:process.env.TYPESAFE_API_KEY,mode:arm,model:'jev-1.13.0'})})});
  const prompt=`Read probe.txt for its current verification marker. Then answer this question from the project records: ${test.query}\nReply in plain text, including the requested decision or values and the verification marker read from the file.`;
  ...
  try{
   ...
   const fullPass=correct&&answer.includes(marker)&&read&&freshRead&&recall===1&&provenance&&preserved&&budget;
   record={id:test.id,category:test.category,arm,initialHash,prompt,evidence,expected:test.expected,forbidden:test.forbidden,status:'done',elapsedMs:Date.now()-start,correct,read,freshRead,recall,provenance,preserved,budget,probeCorrect:answer.includes(marker),fullPass,result};
  }catch(error){
   record={id:test.id,category:test.category,arm,initialHash,status:'error',evidence,elapsedMs:Date.now()-start,error:String(error?.message??error).slice(0,500),fullPass:false,metrics:error?.metrics,trace:error?.trace};
