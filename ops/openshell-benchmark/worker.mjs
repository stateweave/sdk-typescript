...
async function processRun() {
  ...
  state = {
    ...
    labels,
    arms: {
      graph: { slot: armOrder.indexOf("graph") + 1, status: "waiting", sandboxName: sandboxByArm.graph },
      transcript: { slot: armOrder.indexOf("transcript") + 1, status: "waiting", sandboxName: sandboxByArm.transcript }
    }
  };
  ...
  try {
    ...
    for (const arm of armOrder) {
      if (await stopRequested()) throw new StopRequestedError();
      await runArm(arm, sandboxByArm[arm], runDir);
    }
...
async function runArm(arm, sandboxName, runDir) {
  const armState = state.arms[arm];
  ...
  const args = ["sandbox", "exec", "-n", sandboxName, "--timeout", "0", "--workdir", "/sandbox/benchmark/payload/runtime", "--", "env", `PARTICIPANT_MODE=${arm}`, "node", "ops/openshell-benchmark/participant-runner.mjs"];
  ...
  const artifactDir = path.join(runDir, "artifacts", arm);
