...
export class InfiniteAgentHarness {
  ...
  async initialize(): Promise<void> {
    ...
    this.stateweave = new GraphFrameAgent({
      ...
      maxPromptTokens: NAIVE_COMPACTION_THRESHOLD,
...
async function captureStateWeaveTurn(
  ...
  try {
    ...
    return {
      ...
      latencyMs,
      result: { finalAnswer: answer, frame: frame!, graph: frame?.graph ?? { nodes: [], edges: [] }, trace, metadata: error instanceof StateWeaveRunError ? error.metadata : { runId: "error", tools: [], startedAt: new Date(startedAt).toISOString(), completedAt: new Date().toISOString(), durationMs: latencyMs, maxIterations: MAX_AGENT_ITERATIONS, maxPromptTokens: NAIVE_COMPACTION_THRESHOLD, stepCount: trace.length, retryCount: 0, status: "error" } }
    };
  }
}
