...
- Add a primary dual-agent chat where every input concurrently runs unchanged StateWeave and a traditional persistent `messages[]` arm with identical tools in isolated workspaces; atomically persist both outcomes in paired JSONL, preflight compact the traditional transcript at 30K into a validated durable summary plus six messages, distinguish attempted from committed compactions, retain preflight maintenance across failed tasks, and chart both arms' provider input/output, peak context, and compaction cost on one turn axis.
