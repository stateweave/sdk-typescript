...
import { buildChallengerProtocolManifest, CHALLENGER_V6_CORPUS_SHA256, CHALLENGER_V8_PROTOCOL_ID } from "../src/evals/challengerProtocol.js";

it("freezes the balanced Challenger corpus under the sandbox-hardened v8 runtime protocol", async () => {
  const manifest = await buildChallengerProtocolManifest(path.resolve(process.cwd(), "data/challenger-scenarios"));

  expect(manifest.protocolId).toBe(CHALLENGER_V8_PROTOCOL_ID);
